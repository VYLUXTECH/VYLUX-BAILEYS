...
export const generateWAMessageContent = async (message, options) => {
    ...
    // VYLUX-BAILEYS legacy-buttons encoder: ported from wileys 0.7.8 / Baileys 6.7.4.
    // Lets callers pass the wileys convenience shape { text, footer, buttons[], headerType }
    // and have it converted to a proper buttonsMessage proto.
    if ('buttons' in message && !!message.buttons) {
        ...
        if ('text' in message) {
            msg.contentText = message.text;
            msg.headerType = ButtonType.EMPTY;
        }
        else if ('caption' in message) {
            msg.contentText = message.caption;
            const type = Object.keys(m)[0].replace('Message', '').toUpperCase();
            msg.headerType = ButtonType[type];
            Object.assign(msg, m);
        }
        if ('footer' in message && !!message.footer) {
            msg.footerText = message.footer;
        }
        if ('contextInfo' in message && !!message.contextInfo) {
            msg.contextInfo = message.contextInfo;
        }
        if ('mentions' in message && !!message.mentions) {
            msg.contextInfo = { ...(msg.contextInfo || {}), mentionedJid: message.mentions };
        }
        m = { templateMessage: { hydratedTemplate: msg } };
    }
    // VYLUX-BAILEYS list/sections encoder: ported from wileys 0.7.8.
    // Lets callers pass the wileys-style { text, footer, title, buttonText, sections[] }
    // shape and have it converted to a listMessage proto (the "open this list" button).
    ...
    // VYLUX-BAILEYS interactiveButtons encoder: ported from wileys 0.7.8.
    // Lets callers pass the convenience shape { text|caption, footer, title,
    // subtitle, media, interactiveButtons[] } and have it converted to an
    // interactiveMessage.nativeFlowMessage proto (modern native buttons).
